...
T4 is a text template solution in Visual Studio to generate another file on the fly. The T4 file `vaultstructure_ids.tt` in this solution `code-generation-t4` creates a connection to the local `practical-mfiles` vault, reads its vault structure and generates a C# file with IDs, names, aliases for propertyDefs, objectTypes and classes. You can use this C# file with vault structure IDs in your program. :-)

## Using these T4 M-Files code generation in your own programs.
...
## Sample IDs result C# code for the practical-m-files vault

**Sample result C# file with vault structure IDs** - see [vaultstructure-ids.cs](https://github.com/victorvogelpoel/practical-m-files/blob/main/src/metadata-structure/metadata-structure-code-generation-t4/vaultstructure-ids.cs), created on the fly from [vaultstructure-ids.tt](https://github.com/victorvogelpoel/practical-m-files/blob/main/src/metadata-structure/metadata-structure-code-generation-t4/vaultstructure.tt) 
```csharp
using System;
using MFilesAPI;
