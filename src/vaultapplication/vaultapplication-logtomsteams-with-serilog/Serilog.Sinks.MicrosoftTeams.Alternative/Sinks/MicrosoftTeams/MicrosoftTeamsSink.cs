        ...
        private async Task PostMessages(IEnumerable<MicrosoftExtendedLogEvent> messages)
        {
            ...
            foreach (var logEvent in messages)
            {
                try
                {
                    ...
                    //var json = JsonConvert.SerializeObject(message, JsonSerializerSettings);                  // Newtonsoft
                    var json = JsonSerializer.ToJsonString(message, StandardResolver.AllowPrivateCamelCase);  // Utf8Json
                    //var json = JsonSerializer.Serialize(message);                                               // System.Text.Json.JsonSerializer
